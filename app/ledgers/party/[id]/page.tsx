...
export default function PartyLedgerPage({
  ...
  const entries = db
    .prepare(
      `SELECT le.id, v.date, v.number, v.type, v.narration,
              a.name AS account_name, le.debit_paise, le.credit_paise
  ...
  // Debit-positive: a positive closing balance is receivable (they owe us);
  // negative is payable (we owe them).
  const closingLabel = running > 0 ? "Receivable" : running < 0 ? "Payable" : "Settled";
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            {party.gstin ? <span className="mono">{party.gstin} · </span> : null}
            {stateName(party.state_code)} · Party ledger
          </p>
        </div>
      ...
      <form className="toolbar" method="get">
        ...
        {(from || to) && (
          <Link className="btn btn-secondary btn-sm" href={`/ledgers/party/${party.id}`}>
            Clear
          </Link>
        )}
      </form>
      ...
      <div className="card">
        {rows.length === 0 ? (
          <div className="empty">No entries for this party in the selected range.</div>
        ) : (
          <table className="table">
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id}>
                  ...
                  <td>{r.number}</td>
                  <td>{r.type}</td>
                  <td>{r.account_name}{r.narration ? ` · ${r.narration}` : ""}</td>
            ...
            <tfoot>
              <tr>
                <td colSpan={4}>Closing balance ({closingLabel})</td>
