import { salesRegister } from "@/lib/reports";
...
export default function SalesRegisterPage({
  ...
  const { rows, totals } = salesRegister(from, to);
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Sales invoices from {formatDate(from)} to {formatDate(to)}
          </p>
        </div>
      </div>
      ...
      <div className="card">
        {rows.length === 0 ? (
          <div className="empty">No sales invoices in this period.</div>
        ) : (
